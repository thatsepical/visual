local SPAWNER_URL = "https://raw.githubusercontent.com/DeltaGay/femboy/refs/heads/main/GardenSpawner.lua"
local Spawner

local function try(f) local ok,r=pcall(f) return ok and r end

Spawner = try(function() return loadstring(game:HttpGet(SPAWNER_URL, true))() end)

if not Spawner then
    local env = (getgenv and getgenv()) or _G
    if type(env.GardenSpawner) == "table" then
        Spawner = env.GardenSpawner
    elseif type(env.SpawnPet) == "function" then
        Spawner = env
    end
end
...
for _,fn in ipairs({"GetPets","GetSeeds","SpawnPet","SpawnSeed","SpawnEgg","Spin"}) do
    if type(Spawner[fn]) ~= "function" then
        warn("[SpawnerUI] ⚠️ Missing "..fn.." – creating stub")
        Spawner[fn] = function(...) warn("[SpawnerUI] Stub for "..fn) end
    end
end
...
local function makeTab(name,pos)
    local b = Instance.new("TextButton", tabBackground)
    b.Text = name
...
for _,b in ipairs({weightBox,ageBox,amountBox}) do validateDecimal(b) end
...
local function spawnPet()
    ...
    Spawner.SpawnPet(name, tonumber(weightBox.Text) or 1, tonumber(ageBox.Text) or 1)
end
local function spawnSeed()
    local name = seedNameBox.Text
    if name=="" then return end
    Spawner.SpawnSeed(name, tonumber(amountBox.Text) or 1)
end
local function spawnEgg()
    local name = eggNameBox.Text
    if name=="" then return end
    Spawner.SpawnEgg(name)
end
local function spinPlant()
    local name = spinBox.Text
    if name=="" then return end
    Spawner.Spin(name)
end
